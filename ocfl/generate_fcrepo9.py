...
def update_inventory(ocfl_object_path, metadata_list, dry_run):
    ...
    for metadata in metadata_list:
        ...
        for digest in metadata['digests']:
            ...
            #else:
            #    inventory['manifest'][digest].append(f'v1/content/{metadata["contentPath"]}')
#    for metadata in metadata_list:
#        inventory['manifest'].update({digest: [f'v1/content/{metadata["contentPath"]}'] for digest in metadata['digests']})
    # Add the .fcrepo files to the 'manifest' section
    fcrepo_files = [
    ...
    for file_path in fcrepo_files:
       ...
       # if file_hash not in inventory['manifest']:
        inventory['manifest'][file_hash] = [file_path]
       # else:
       #     inventory['manifest'][file_hash].append(file_path)
    
    # Update the 'version/v1/state' section to match the 'manifest'
