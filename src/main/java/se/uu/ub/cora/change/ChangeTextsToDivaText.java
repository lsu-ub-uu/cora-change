...
public class ChangeTextsToDivaText {
	...
	public void changeValidationTypeForDivaTexts() {
		...
		for (ClientData clientData : listOfTexts.getDataList()) {
			...
			if ("systemOne".equals(textRecordGroup.getDataDivider())) {
				handleDivaText(textRecordGroup, id);
			} else {
				System.out.println("id: " + id);
			}
			//
			// textRecordGroup.removeChildrenWithTypeAndName(ClientDataRecordLink.class,
			// "validatesRecordType");
			// ClientDataRecordLink newLink = ClientDataProvider
			// .createRecordLinkUsingNameInDataAndTypeAndId("validatesRecordType",
			// "recordType", id);
			//
			// textRecordGroup.addChild(newLink);
			//
			// dataClient.update("validationType", id, textRecordGroup);
		}
	}

	private void handleDivaText(ClientDataRecordGroup textRecordGroup, String id) {
		systemOutPrintlnBoldGreen("id: " + id);
		ClientDataGroup recordInfo = textRecordGroup.getFirstGroupWithNameInData("recordInfo");
		ClientDataRecordLink validationType = recordInfo
				.getFirstChildOfTypeAndName(ClientDataRecordLink.class, "validationType");
		systemOutPrintlnBoldGreen(validationType.getLinkedRecordId());
		if (!"textSystemOne".equals(validationType.getLinkedRecordId())) {
			systemOutPrintlnBoldGreen(validationType.getLinkedRecordId());
			ClientDataRecordLink newValidationType = ClientDataProvider
					.createRecordLinkUsingNameInDataAndTypeAndId(validationType.getNameInData(),
							validationType.getLinkedRecordType(), "textSystemOne");
			textRecordGroup.removeChildrenWithTypeAndName(ClientDataRecordLink.class, "recordInfo");
			textRecordGroup.addChild(newValidationType);
			systemOutPrintlnBoldGreen("changed validationType... updating record with id: " + id);
